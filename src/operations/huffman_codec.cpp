...
void HuffmanCodec::compress(Tree *tree, std::string &firstLine, std::string &compressed,
    ...
    for(int i = 0; i < compressedBits.size(); i += BYTE_LEN) {
        ...
        if (compressedByte.size() < BYTE_LEN) {
            /* If the number of bits is not a multiplier of BYTE_LEN, we add as many "0"s as required to make
             * the last chunk as long as BYTE_LEN. */
